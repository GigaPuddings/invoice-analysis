...
// 解析PDF提取的文本位置信息
#[tauri::command]
async fn parse_invoice_text(filename: &str, pages_text_items: Vec<Vec<TextItem>>) -> Result<Vec<Invoice>, String> {
...
fn extract_nearby_text(
    ...
    // 找到参考文本项
    let ref_item = text_items.iter().find(|item| reference_text.is_match(&item.text));
    if ref_item.is_none() {
        return "".to_string();
    }
    ...
    // 根据方向筛选候选文本项
    let mut candidates: Vec<&TextItem> = text_items.iter().filter(|item| {
        // 排除参考项自身
...
fn extract_party_info(
    ...
    // 查找底部边界的文本项
    let footer_item = text_items.iter().fold(None, |result: Option<&TextItem>, item| {
    ...
    let offset_y = if invoice.invoice_type == "普通发票" { 0.0 } else { 8.0 };
    ...
    // 查找区域内的所有文本项
    let area_items: Vec<&TextItem> = text_items.iter().filter(|item| {
    ...
    // 设置到相应的对象
    let party_obj = if is_seller_info { &mut invoice.seller } else { &mut invoice.buyer };
...
fn extract_invoice_items(text_items: &[TextItem], invoice: &mut Invoice) {
    ...
    let name_header_item = text_items.iter().find(|item| {
        item.text.contains("货物") || item.text.contains("项目")
    });
    ...
    let model_bottom_item = text_items.iter().find(|item| {
        item.text == "合" || item.text == "合计"
    });
    ...
    let value_items: Vec<&TextItem> = area_items.iter().filter(|item| {
        item.y >= no_title
    }).copied().collect();
    ...
    for (i, row) in grouped_rows.iter().enumerate() {
        ...
        for (index, text_item) in row.iter().enumerate() {
            ...
            if row.len() > 5 && (index == row.len() - 5 || index == row.len() - 4) {
                ...
                // 金额，只有当还没设置时才赋值
                let is_price_format = regex::Regex::new(r"^[¥￥]?-?[\d.]+$").unwrap().is_match(value);
...
fn extract_total_amount_and_tax(text_items: &[TextItem], invoice: &mut Invoice) {
    let candidate_item = text_items.iter().find(|item| item.text == "计" || item.text == "合计");
    ...
    while i < sorted_items.len() {
        ...
        let is_amount_format = regex::Regex::new(r"^[¥￥]?\d+(\.\d+)?$").unwrap().is_match(text);
        ...
        if is_amount_format {
            ...
            if i + 1 < sorted_items.len() {
                ...
                let is_next_amount = regex::Regex::new(r"^\d+(\.\d+)?$").unwrap().is_match(next_text);
                if is_next_amount {
...
fn parse_generic_fapiao(
    ...
    let title_item = text_items.iter().find(|item| title_regex.is_match(&item.text));
